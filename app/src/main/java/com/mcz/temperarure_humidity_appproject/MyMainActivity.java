...
public class MyMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mQQHealthView= (HealthView) findViewById(R.id.qqHealthView);
        mQQHealthView2= (HealthView) findViewById(R.id.qqHealthView2);
        mQQHealthView3= (HealthView) findViewById(R.id.qqHealthView3);
    ...
    Runnable   dataUi=new  Runnable(){
        ...
        public void run() {
            ...
            adapter.settoken(token);
            for (int i=0;i<mlist.size();i++){
                Log.i("xinlv67",mlist.get(i).getDevicetemperature());
            }

            //mQQHealthView.setSteps();
    ...
    private void init(){
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (!(sp.getString("nowgatewayid","").equals(""))){
                    ListviewADD_DataHis();
                }
            }
        }).start();
                        ...
                        if (k==0){
                            if (returndata2==2){
                                ...
                                for (int i = 0; i<gatewayId.size(); i++){
                                    list.add(gatewayId.get(i));
                                }
                                String[] DialogData=list.toArray(new String[0]);
                                new BottomPopUpDialog.Builder()
                                        ...
                                        .setItemOnListener(new BottomPopUpDialog.BottomPopDialogOnClickListener() {
                                            ...
                                            public void onDialogClick(String tag) {
                                                sp.edit().putString("nowgatewayid",tag).commit();
                                                Toast.makeText(MyMainActivity.this,"选择"+tag,Toast.LENGTH_SHORT).show();
                                            }
                                        })
...
*/
    ...
    private List<DataInfo> ListviewADD_DataHis() {
        returndata=0;
        new Thread(new Runnable() {
            @Override
            public void run() {
...
//                    + "&pageNo=" + Fnum + "&pageSize=" + Onum;
                    ////////////////////////////////////////////************************查询设备历史数据*****************/////////////////////////////////////
                    String add_url = Config.all_url + "/iocm/app/data/v1.1.0/deviceDataHistory?deviceId="+sp.getString("nowgatewayid","")+"&gatewayId="+sp.getString("nowgatewayid","");
                    Log.i("zzza","123");
                    String json = DataManager.Txt_REQUSET(MyMainActivity.this, add_url, login_appid, token);
...
//                dataInfo.setDevicetimestamp(timestamp);
                        JSONObject jsonObject = new JSONObject(ser_data);
                        dataInfo.setDevicehumidity(jsonObject.optString("xinlv"));
                        dataInfo.setDevicetemperature(jsonObject.optString("xueya"));
                        dataInfo.setDevicetiwen(jsonObject.optString("tiwen"));
                        mlist.add(dataInfo);
                    }
                    returndata=1;//已经成功获取
